...
def extract_entities(question):
    """
    Use LLM to extract entities from the question.

    Returns a list of dicts:
      [{"name": "...", "type": "PERSON/ORG/WORK/OTHER"}, ...]
    """
    prompt = f"""
You are analyzing a question for multi-hop reasoning.

Identify:
1. The ANSWER TARGET: what kind of thing is being asked for (e.g., "city", "date", "government position").
2. All MENTIONED PARTICIPANTS: this includes BOTH
   - named entities (people, places, organizations, works), and
   - descriptive phrases that refer to an unnamed person/thing (e.g., "the woman who...", "the company that...", "the author of...").

For each participant, include:
- "name_or_description": the exact phrase in the question,
- "role": brief description of its role in the question (e.g., "unknown person", "movie title", "character name").

Return STRICT JSON:
{{
  "answer_target": "<short noun phrase>",
  "participants": [
    {{
      "name_or_description": "...",
      "role": "..."
...
"""
    ...
    data = extract_json(raw, fallback={"entities": []})

    entities = data.get("entities", [])
    # Basic cleanup: dedupe by name
    seen = set()
    cleaned = []
    for e in entities:
